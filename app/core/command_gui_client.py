...
class CommandGUIClient:
    def __init__(self,receiver_port = None, server_host='localhost', server_port=5001, reconnect_interval=5):
        ...
        self.socket = None
        self.receiver_thread = threading.Thread(target=self.receive_response, daemon=True)
    ...
    def connect(self, max_retries=5):
        ...
        while not self._stop_event.is_set() and attempt < max_retries:
            try:
                ...
                self.receiver_host = self.socket.getsockname()[0]
    ...
    def connection_as_observer(self):
        connect_command = {
            'type': 'connect',
            'client_type': 'observer',
            'host': self.receiver_host,
